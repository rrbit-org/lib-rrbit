...
import {
	...
	makeTransientSizes,
	setupNewBlockInInitBranch} from './shared/tree';
 ...
 * @return {Vector<T>}
 */
export function prepend(value, list, create) {

	if (list.length === 0) {
		var vec = empty(create);
		vec.length = 1;
		vec.display0 = [value];
		return vec;
	}

	var vec = fromFocusOf(list, create);
	vec.length = list.length;
	vec.transient = list.transient;
	vec.length = list.length + 1

	focusOnFirstBlockǃ(vec)
	if (list.display0.length < 32) { // element fits in current block
		prependOnCurrentBlock(value, vec)
	} else {
		prependFrontNewBlock(value, vec)
	}
