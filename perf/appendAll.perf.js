...
describe('concat performance comparisons', function() {
	...
	it('rrbit appendǃ + speed', function() {
		...
		var vec = api.empty();
		iterator(0, list1k.rrbit.length, list1k.rrbit).reduce(function(list, val) {
			return list.appendǃ(val, list);
		}, vec);

		iterator(0, list2k.rrbit.length, list2k.rrbit).reduce(function(list, val) {
			return list.appendǃ(val, list);
		}, vec);

		iterator(0, list3k.rrbit.length, list3k.rrbit).reduce(function(list, val) {
			return list.appendǃ(val, list);
		}, vec);
	});
